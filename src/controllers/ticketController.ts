...
export const createTicket = async (req: Request, res: Response, next: NextFunction) => {
    try{
        ...
        if (!user) {
            res.status(401).json({
                success: false,
                message: "You are  not uthorized to perform this action",
                data: null
            });
            return;
        }
        ...
        sendEmail(
            ...
            `A new support ticket has been created by user ${user.fullName}. Subject: ${subject}. Description: ${description}`
        )
...
export const addReplyToTicket =  async (req: Request, res: Response, next: NextFunction) => {
    try{
